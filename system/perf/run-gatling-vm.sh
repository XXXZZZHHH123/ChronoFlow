#!/usr/bin/env bash
# Run Gatling performance tests inside a Docker container, persisting reports locally.
set -euo pipefail
...
IMAGE_NAME="${IMAGE_NAME:-chronoflow-system-gatling:latest}"
DOCKERFILE_PATH="${DOCKERFILE_PATH:-system/perf/Dockerfile}"
CONTAINER_RESULTS_PATH="/workspace/system/target/gatling"
...
docker_args+=("-e" "TASK_SERVICE_BASE_URL=${BASE_URL}")
...
declare -a optional_vars=(
  ...
  SYSTEM_PERF_TASK_ID
  SYSTEM_PERF_RESULTS_UPLOAD_URL
  ...
  TASK_SERVICE_RESULTS_UPLOAD_URL
  TASK_SERVICE_RESULTS_OBJECT
)

for optional_var in "${optional_vars[@]}"; do
  if [[ -n "${!optional_var:-}" ]]; then
    case "${optional_var}" in
      SYSTEM_PERF_*)
        legacy_var="TASK_SERVICE_${optional_var#SYSTEM_PERF_}"
        docker_args+=("-e" "${legacy_var}=${!optional_var}")
        ;;
      *)
